 ...
 * servidor_grupo_m2.c
 *
 * Servidor de grupo 1 para la arquitectura de mensajería instantánea IPv6.
 *
 * Este servidor se ejecuta en M2.
 *
 * Funciones:
 *   - Escucha mensajes TCP en el puerto 4001.
 *   - Recibe mensajes enviados por los clientes.
 *   - Guarda los mensajes en el fichero grupo_1.log.
 *   - Reenvía los mensajes por UDP multicast IPv6 a ff15::1:5001.
 *
 * Compilación:
 *   gcc -Wall -Wextra -o servidor_grupo_m2 servidor_grupo_m2.c
 *
 * Ejecución en M2:
 *   ./servidor_grupo_m2
 */
...
#define MULTICAST_IPV6 "ff15::1"
#define MULTICAST_PORT 5001

#define LOG_FILE "grupo_1.log"
...
static int save_message_to_file(const char *message)
{
    ...
    if (file == NULL) {
        perror("[M2] Error abriendo fichero grupo_1.log");
        return -1;
    }
    ...
    if (fflush(file) != 0) {
        perror("[M2] Error haciendo fflush del histórico");
        fclose(file);
        return -1;
    }

    if (fclose(file) != 0) {
        perror("[M2] Error cerrando fichero del histórico");
        return -1;
    }
...
static int create_tcp_server_socket(void)
{
    ...
    if (server_fd < 0) {
        perror("[M2] Error creando socket TCP IPv6");
        return -1;
    }
    ...
    if (setsockopt(server_fd,
                   ...
                   &optval,
                   sizeof(optval)) < 0) {
        perror("[M2] Error en setsockopt SO_REUSEADDR");
        close(server_fd);
        return -1;
    }
    ...
    if (bind(server_fd,
        ...
        perror("[M2] Error en bind TCP");
        close(server_fd);
        return -1;
    }

    if (listen(server_fd, BACKLOG) < 0) {
        perror("[M2] Error en listen");
        close(server_fd);
        return -1;
    }
...
static int create_multicast_socket(void)
{
    ...
    if (udp_fd < 0) {
        perror("[M2] Error creando socket UDP IPv6");
        return -1;
    }
    ...
    if (setsockopt(udp_fd,
                   ...
                   &hops,
                   sizeof(hops)) < 0) {
        perror("[M2] Aviso: no se pudo configurar IPV6_MULTICAST_HOPS");
    }
...
static int send_multicast_message(int udp_fd, const char *message)
{
    ...
    if (inet_pton(AF_INET6,
        ...
        fprintf(stderr,
                "[M2] Dirección multicast IPv6 inválida: %s\n",
                MULTICAST_IPV6);
        return -1;
    }
    ...
    if (sent < 0) {
        perror("[M2] Error enviando por multicast");
        return -1;
    }
                          ...
                          int udp_fd)
{
    ...
    printf("[M2] Cliente conectado desde [%s]:%d\n",
           client_ip,
           client_port);
    ...
    if (bytes_read < 0) {
        perror("[M2] Error en recv");
        close(client_fd);
        return;
    }

    if (bytes_read == 0) {
        printf("[M2] Cliente cerró la conexión sin enviar datos\n");
        close(client_fd);
        return;
    }
    ...
    printf("[M2] Mensaje recibido por TCP:\n%s",
           final_message);

    if (save_message_to_file(final_message) == 0) {
        printf("[M2] Mensaje guardado en %s\n", LOG_FILE);
    } else {
        printf("[M2] No se pudo guardar el mensaje en %s\n", LOG_FILE);
    }

    if (send_multicast_message(udp_fd, final_message) == 0) {
        printf("[M2] Mensaje reenviado por multicast a [%s]:%d\n",
               MULTICAST_IPV6,
               MULTICAST_PORT);
    } else {
        printf("[M2] No se pudo reenviar el mensaje por multicast\n");
    }

    const char *ack = "OK mensaje_recibido\n";

    if (send(client_fd, ack, strlen(ack), 0) < 0) {
        perror("[M2] Error enviando ACK");
    }

    close(client_fd);

    printf("[M2] Conexión cerrada\n\n");
}
...
int main(void)
{
    ...
    printf("=============================================\n");
    printf(" Servidor M2 - Grupo 1 iniciado\n");
    ...
    printf("\n[M2] Cerrando servidor de grupo 1...\n");
